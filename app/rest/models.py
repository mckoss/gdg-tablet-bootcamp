...
class RESTModel(HasReadOnly, db.Model):
    ...
    def get_read_only(cls):
        ...
        return ('owner_email',) + super(RESTModel, cls).get_read_only()
    ...
    def get_dict(self, depth=1):
        """ Returns JSON-compatible python dictionary containing all the
    ...
    def set_dict(self, json_dict):
        """ Sets the properties on a model instance from a
        ...
        for prop_name, prop in self.properties().iteritems():
            ...
            if value is not None and prop.data_type is float:
                ...
                # this probably works
                value = datetime.utcfromtimestamp(float(value) / 1000)
                ...
                if other is None:
                    raise ValueError("Invalid reference %s[%s]." %
                                     (prop.data_type.kind(), value))
                else:
                    value = key
            else:
                raise ValueError('cannot decode %s: %r (type %s)' %
                                 (prop_name,
                                  value,
                                  prop.data_type))
...
def py_to_js_type(py_type):
    """ Return { "type": "<JavaScript data type>",
    "control": "<HTML input type>" } """
